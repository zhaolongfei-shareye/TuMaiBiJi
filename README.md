...
1. ~~**自建 OCR 换库 + 凭据收口**~~ → **本轮已做完代码侧（60 项测试全绿），未部署**。剩余的是**服务器侧四件事**：`df -h` 看磁盘够不够 273MB、装新依赖前单跑 `import cv2`、部署当天读缩放后的峰值 RSS、真机走一遍"截图 → 认字 → 入库 → 列表可见"。**AppSecret 外泄那条（R14）不在这四件事里，已于 2026-09-19 22:37 单独上线并复验通过**。**`.env` 装载那两条（R15 / R16 / §8.8 / §8.9）也不在这四件事里，但它们不能照 R14 那样单点上线**——服务器实况已核对：线上 `config.py` 17 个字段，`ocr.py`/`asr.py`/`storage.py` 共 18 处在读 `TENCENT_OCR_*`/`TENCENT_ASR_*`/`COS_*`，而仓库版这些字段已随 COS/腾讯云 OCR 出局删掉；只传 `config.py` 会让现网 API 在 import 阶段 `AttributeError` 起不来。可选路径见 PRD §8.9 末尾（随整批一起上 / 给线上那份打只加不改的最小补丁），**这条推翻了我上一轮"理论上可以单点上线"的说法**。
