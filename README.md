...
跑测试：`pytest -q`（38 项 = 原有闭环 19 + 自建 OCR 19；OCR 相关用例不需要 Redis、不需要网络、不需要任何密钥）
...
1. ~~**自建 OCR 换库 + 凭据收口**~~ → **本轮已做完代码侧（38 项测试全绿），未部署**。剩余的是**服务器侧四件事**：`df -h` 看磁盘够不够 273MB、装新依赖前单跑 `import cv2`、部署当天读缩放后的峰值 RSS、真机走一遍"截图 → 认字 → 入库 → 列表可见"
