...
5. **出一个体验版给手机扫**（R17 修完后就该做这件事，用户已批准）：现网 `1.0.0` 里没有 `d449dc8`（中性文案 + 格式提示）和 `c323e49`（toast 送达）这两条前端修复，也没有"真机走一遍"这条待办要验的东西。上传方式优先 `miniprogram-ci` + 仓库外的 `private.wx4416d1283b5de721.key`；若被 IP 白名单挡住，就退回让用户在开发者工具里点一次上传。**顺带要定：`pages/about/about.js:5` 那个显示用的 `version: '1.0.0'` 是否随体验版一起改，否则手机上看到的版本号分不清是哪个包**
