...
- ✅ **截图认字已换成自建开源 RapidOCR**：`services/ocr.py` 从腾讯云 TC3 签名整体重写为进程内调用，**对外两个函数签名一字未改**，worker 与既有测试零改动。落实了 4 条实测要求：识别前缩放长边 ≤1600、单张串行（线程锁）、按检测框坐标重排为视觉顺序、过滤噪声行。**本机同图对照：峰值 RSS 1,076.6MB → 666.6MB，识别字符数 294 → 301**（不是拿推理凑的）。新增 18 条测试（审查后再补 5 条：暂存的每用户批次数与全局预算、坏图给中文字段），与原有 19 条合并 **37 passed**
...
│   ├── tests/test_ocr_flows.py  # 自建 OCR 的 18 项：缩放 / 排序 / 降噪 / 串行 / 批次与暂存上限 / 坏图中文字段 / 失败态
...
跑测试：`pytest -q`（37 项 = 原有闭环 19 + 自建 OCR 18；OCR 相关用例不需要 Redis、不需要网络、不需要任何密钥）
...
1. ~~**自建 OCR 换库 + 凭据收口**~~ → **本轮已做完代码侧（37 项测试全绿），未部署**。剩余的是**服务器侧四件事**：`df -h` 看磁盘够不够 273MB、装新依赖前单跑 `import cv2`、部署当天读缩放后的峰值 RSS、真机走一遍"截图 → 认字 → 入库 → 列表可见"
